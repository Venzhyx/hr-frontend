...
const AddTimeOffPage = () => {
  ...
  // ID karyawan milik user yang login (untuk EMPLOYEE)
  ...
  useEffect(() => {
    if (!isAdmin && selfEmployeeId) {
      setForm(f => ({ ...f, employeeId: String(selfEmployeeId) }));
    }
  ...
  useEffect(() => {
    if (!isAdmin && selfEmployeeId) {
      // Coba fetch langsung via endpoint individual
      getEmployeesAPI()
        .then(res => {
          const list = res.data?.data ?? res.data ?? [];
          const me = Array.isArray(list)
            ...
            : null;
          if (me) {
            setSelfEmployeeName(me.fullName ?? me.name ?? me.fullname ?? "");
          }
        })
        .catch(() => {});
    }
    ...
    <>
      ...
      <div className="w-full px-4 md:px-6 py-6">
        ...
        <div className="space-y-4">
          ...
          {/* ══ Section 1: Employee + Time Off Type ══ */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div>
                ...
                {isAdmin ? (
                  ...
                  <div className="relative">
                    <div className={`${inputCls} flex items-center justify-between bg-gray-100 cursor-not-allowed`}>
                      ...
                      <HiOutlineLockClosed className="w-3.5 h-3.5 text-gray-400 flex-shrink-0 ml-2" />
                    </div>
                    {/* hidden input agar form.employeeId tetap terkirim */}
                    <input type="hidden" value={form.employeeId} readOnly />
                  </div>
                )}
              </div>
