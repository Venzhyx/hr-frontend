 ...
 * Mendukung key "user" maupun "hr_user".
 */
...
const TimeOffTablePage = () => {
  ...
  // ── Ambil user yang login & tentukan role ──
  ...
  useEffect(() => {
    load();
    // Hanya fetch daftar karyawan kalau ADMIN (untuk photo map & kolom nama)
   ...
   *   - Cocokkan dengan loggedInUser.employeeId  (jika ada)
   *   - Fallback ke loggedInUser.id
  ...
  // Stats (dihitung dari ownRequests agar angka konsisten)
  const stats = ownRequests.reduce(
    (acc, r) => {
      acc.total++;
  ...
  const counts = ownRequests.reduce((acc, r) => {
  ...
  const filteredData = useMemo(
    ...
    [ownRequests, activeFilters, search]
  );
          ...
          <div>
            ...
            {!isAdmin && (
              <p className="text-sm text-gray-500 mt-0.5">
                Menampilkan permohonan cuti milik Anda
              </p>
            )}
          </div>
            ...
            <div className="p-6">
              <div className="flex items-center mb-4">
                ...
                <p className="text-gray-700">
                  Yakin ingin menghapus time off request
                  {isAdmin && (
                    <>
                      {" "}dari{" "}
